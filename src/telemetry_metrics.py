...
def run(src_csv, dst_csv=None):
    df = pd.read_csv(src_csv).sort_values("timestamp")
    dt   = df["timestamp"].diff()
    df["accel"] = df["speed_kmh"].diff() / dt
    df["jerk"]  = df["accel"].diff()     / dt
    out = pathlib.Path(dst_csv or pathlib.Path(src_csv).with_name("telemetry_metrics.csv"))
